...
> **Status (Aug 2026):** Tier 1 #1 (optimistic concurrency + rows-affected check) and #2 (explicit transactions) shipped in **1.6.0**; FK constraint DDL + honored cascade behaviors + rename advisories shipped in **1.7.0**; #3 `executeUpdate`/`executeDelete` + set-based `bulkUpdate`/`bulkDelete` shipped in **1.8.0**; #4 named global query filters and #10 context events/interceptors (+ `command` observer) shipped in **1.9.0**; #5 retry/resiliency and #6 pluggable logging with parameter redaction shipped in **1.10.0**; #7 migrations tooling (atomic apply, `script`, `migrations-status`, `--connection`, latest-migration id) shipped in **1.11.0**; #8 `find`, #9 `entry()`/`hasChanges()`/`entries()` and most of #11 (`sum/avg/min/max`, `thenBy`, `distinct`, `any`, DTO list, projected `pluck`, loud `join`/`groupBy`) plus `ctx.add/remove` shipped in **1.12.0**; the three addendum bugs (lazy navigation via un-awaited SQLite-shaped SQL → engine-agnostic explicit/lazy loading with fix-up, async `RedisQueryCache` not awaited, readme `.any()`/`context.remove()` mismatches) shipped in **1.13.0**; Tier-3 DDL modeling (`defaultSql` = HasDefaultValueSql, `computed` = HasComputedColumnSql, `check` = HasCheckConstraint, generated values read back after insert) shipped in **1.14.0**; health checks on every engine (`healthCheck`/`canConnect`) and `remove-migration` (EF `migrations remove`, `--force` reverts first) shipped in **1.15.0**; many-to-many skip navigations (`manyToMany()` with a synthesized join entity, collection `add`/`remove`, `entry().collection()/reference()`) shipped in **1.16.0**; `thenInclude()` / `asSplitQuery()` as EF split queries shipped in **1.17.0**; query filters applied inside `include()` with `ignoreQueryFilters()` propagating to every level shipped in **1.18.0**; TPH inheritance (`dbset(Cat, { extends: Animal })`, discriminator, derived sets, materialization as derived types) shipped in **1.19.0**; composite primary keys (`HasKey(a, b)`: DDL, keyed UPDATE/DELETE, `find(a, b)`) shipped in **1.20.0**; `groupBy().aggregate()` (GROUP BY / HAVING translation) shipped in **1.21.0**; owned/complex JSON types (`db.owned(Class)`, DetectChanges on nested mutations) shipped in **1.22.0**. Still open (by design): `join()`/`leftJoin()` LINQ-style joins — use `include()`/`thenInclude()` or `ctx.query()`; FKs to composite-key entities; `manyToMany()` on composite-key owners. Previously listed as open: `thenInclude`, `groupBy`/`join` translation, and Tier 3 modeling. Remaining items are tracked below in their original ranking.
